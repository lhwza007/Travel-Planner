...
export const InsertParkPlaces = async (req, res) => {
  ...
  try {
    ...
    const q =
      "INSERT INTO parkplaces(park_id, parkplace_lat, parkplace_lng, parkplace_name, parkplace_description) VALUES(?, ?, ?, ?, ?)";

    await db
      .promise()
      .query(q, [park_id, parkplace_lat, parkplace_lng, parkplace_name, parkplace_description]);

    return res.status(200).json("ParkPlaces has been created.");
  } catch (error) {
